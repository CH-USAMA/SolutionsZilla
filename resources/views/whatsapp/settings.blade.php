                ...
                @else
                    ...
                    <form method="POST" action="{{ route('whatsapp.settings.update') }}" class="space-y-6" x-data="{ 
                        ...
                        @if(count($allowedProviders) === 0 && !Auth::user()->isSuperAdmin())
                            <p class="mt-1 text-sm text-red-500 italic">No providers enabled for your clinic. Please contact
                                support.</p>
                        @endif
                                ...
                                @if(in_array('js_api', $allowedProviders) || Auth::user()->isSuperAdmin())
                                    <div class="flex flex-col items-center" x-show="provider === 'js_api'">
                                        <div id="qr-container" class="bg-white p-4 rounded-lg shadow-inner mb-4">
                                            @if($hasJsUrl)
                                                <div
                                                    class="w-48 h-48 bg-gray-100 flex items-center justify-center text-gray-400 italic text-sm">
                                                    Loading QR Code...
                                                </div>
                                            @else
                                                <div
                                                    class="w-48 h-48 flex flex-col items-center justify-center text-gray-500 p-4 text-center text-xs">
                    ...
                    <div class="mt-8">
                        ...
                        <p class="text-sm text-gray-600 mb-6">Send a sample message to verify your connection is working
                            correctly.
                        </p>
                        ...
                        <div class="bg-gray-50 p-6 rounded-xl border border-gray-100">
                            <form method="POST" action="{{ route('whatsapp.test') }}" class="space-y-4">
                                ...
                                <div class="max-w-md">
                                    ...
                                    <div class="flex mt-1 gap-2">
                                        <x-text-input id="test_phone" name="test_phone" type="text" class="block w-full"
                                            placeholder="e.g. 923001234567" required :value="old('test_phone')" />
                            ...
                            @if(Auth::user()->isSuperAdmin())
                                <div class="mt-6 pt-6 border-t border-gray-200">
                                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Developer Tools
                                    </h4>
